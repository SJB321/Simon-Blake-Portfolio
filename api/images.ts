// /api/images
//   GET   → list every image in the bucket (admin-gated)
...
async function getList(req: VercelRequest, res: VercelResponse) {
...
async function postUpload(req: VercelRequest, res: VercelResponse) {
  if (!(await requireAuth(req, res))) return
  ...
  if (buffer.byteLength > MAX_BYTES) {
    return res.status(413).json({
      error: `File too large (${(buffer.byteLength / 1024 / 1024).toFixed(
        2,
      )} MB). Max is 4 MB.`,
    })
  }
