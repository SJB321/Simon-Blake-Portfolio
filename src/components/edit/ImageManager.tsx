...
import type { ImageItem, ResumePayload } from '../../types/resume'
 ...
 */
function findReferences(data: ResumePayload | null, url: string | null): string[] {
  if (!data || !url) return []
  const refs: string[] = []
...
export default function ImageManager({ password }: ImageManagerProps) {
...
function ImagePickerModal({ password, onPick, onClose }: ImagePickerModalProps) {
  const [images, setImages] = useState<ImageItem[] | null>(null)
  const [error, setError] = useState<string | null>(null)
